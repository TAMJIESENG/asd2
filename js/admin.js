...
function initChatSystem() {
    ...
    // 设置定时刷新
    setInterval(loadChatUsers, 10000);
    
    // 搜索用户
    if (chatSearch) {
        chatSearch.addEventListener('input', function() {
            const searchTerm = this.value.toLowerCase().trim();
            filterChatUsers(searchTerm);
        });
    }
    ...
    function loadChatUsers() {
        ...
        // 生成用户列表
        const userList = Object.keys(userMessages).map(userId => {
            const messages = userMessages[userId].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
        ...
        // 按最后消息时间排序，最新的在前面
        userList.sort((a, b) => new Date(b.lastMessage.timestamp) - new Date(a.lastMessage.timestamp));
        
        // 渲染用户列表
        renderChatUsers(userList);
        
        // 如果有当前选中的用户，刷新消息显示
        if (currentChatUser) {
            loadUserMessages(currentChatUser);
        }
    }
    
    // 渲染聊天用户列表
    function renderChatUsers(userList) {
        if (!chatUserList) return;
        
        // 保存当前选中的用户ID
        const currentUserId = currentChatUser ? currentChatUser.userId : null;
        
        // 生成HTML
        let html = '';
        userList.forEach(user => {
            const lastMessage = user.lastMessage;
            const isActive = currentUserId === user.userId;
            
            html += `
                <div class="chat-user-item ${isActive ? 'active' : ''}" data-user-id="${user.userId}">
                    <div class="chat-user-avatar">${getUserInitials(user.userId)}</div>
                    <div class="chat-user-info">
                        <div class="chat-user-name">用户 ${getUserDisplayName(user.userId)}</div>
                        <div class="chat-last-message">${lastMessage ? truncateText(lastMessage.content, 25) : ''}</div>
                    </div>
                    <div class="chat-user-meta">
                        <div class="chat-time">${formatChatTime(lastMessage.timestamp)}</div>
                        ${user.unreadCount > 0 ? `<div class="chat-badge">${user.unreadCount}</div>` : ''}
                    </div>
                </div>
            `;
        });
        ...
        if (userList.length === 0) {
            html = '<div class="no-chat-users">暂无聊天用户</div>';
        }
    ...
    function selectChatUser(user) {
        ...
        if (currentChatUsername) {
            currentChatUsername.textContent = `用户 ${getUserDisplayName(user.userId)}`;
        }
        
        // 更新在线状态
        if (chatStatus) {
            chatStatus.className = 'chat-status';
            chatStatus.classList.add('online'); // 这里假设所有用户都在线，实际应用中需要根据状态设置
        }
    ...
    function addMessageToUI(message) {
        ...
        messageElement.className = `chat-message ${message.sender === 'admin' ? 'outgoing' : 'incoming'}`;
        ...
        if (message.sender === 'user') {
            contentHTML = `
                <div class="chat-message-avatar">${getUserInitials(message.userId)}</div>
            ...
            contentHTML = `
                ...
                <div class="chat-message-avatar">管理</div>
            `;
        }
    ...
    function markMessagesAsRead(userId) {
        ...
        const updatedMessages = allMessages.map(message => {
            if (message.userId === userId && message.sender === 'user' && !message.isRead) {
...
function getUserInitials(userId) {
    // 从userId中提取首字母
    const shortId = userId.split('_')[1];
    return shortId ? shortId.substring(0, 2).toUpperCase() : 'U';
}

// 获取用户显示名称
function getUserDisplayName(userId) {
    // 从userId中提取一个短ID作为显示名称
    const shortId = userId.split('_')[1];
    return shortId ? shortId.substring(0, 6) : userId;
}
